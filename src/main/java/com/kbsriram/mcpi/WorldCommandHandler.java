...
{
	public final static class GetBlock implements ICommandHandler
	{
	...
	public final static class GetBlocksWithData implements ICommandHandler
	{
		public String handle(Command cmd, WorldServer ws) throws Exception
		{
			if (cmd.getArgs().length != 6)
			{
				return "usage: world.getBlocksWithID(x1,y1,z1,x2,y2,z2)";
			}
			WorldInfo info = ws.getWorldInfo();
			...
			StringBuilder blocks = new StringBuilder((x2-x1) * (y2-y1) * (z2-z1) * 3);
			...
			for (int x = x1; x <= x2; x++)
			{
				for (int z = z1; z <= z2; z++)
				{
					...
					{
						Block b = ws.getBlock(x, y, z);
						String bid = String.valueOf(Block.getIdFromBlock(b));
						int meta = ws.getBlockMetadata(x, y, z);
						blocks.append(bid);
						blocks.append(":");
						blocks.append(Integer.toString(meta));
						blocks.append(",");
					}
				}
			}			
			...
			if(blocks.length() > 0)
			{
				blocks.deleteCharAt(blocks.length()-1);
			}
	...
	public final static class GetBlocks implements ICommandHandler
	{
		public String handle(Command cmd, WorldServer ws) throws Exception
		{
			if (cmd.getArgs().length != 6)
			...
			for (int x = x1; x <= x2; x++)
			{
				for (int z = z1; z <= z2; z++)
				{
					for (int y = y1; y <= y2; y++)
					{	
						blocks.append(String.valueOf(Block.getIdFromBlock(ws.getBlock(x, y, z))));						
						blocks.append(",");
					}
				}
			}			
	...
	public final static class GetHeight implements ICommandHandler
	{
	...
	public final static class SetBlock implements ICommandHandler
	{
		public String handle(Command cmd, WorldServer ws) throws Exception
		{
			String[] args = cmd.getArgs();
	...
	public final static class SetTileEntityHex implements ICommandHandler
	{
		public String handle(Command cmd, WorldServer ws) throws Exception
		{
			String[] args = cmd.getArgs();
			...
			NBTTagCompound tag = CompressedStreamTools.readCompressed(new ByteArrayInputStream(hex));
			tag.setInteger("x", x);
			tag.setInteger("y", y);
			tag.setInteger("z", z);
	...
	public final static class SetBlocks implements ICommandHandler
	{
		public String handle(Command cmd, WorldServer ws) throws Exception
		{
			String[] args = cmd.getArgs();
	...
	public final static class GetPlayerIds implements ICommandHandler
	{
